...
async function bbonous() {
  window.onload = async function price() {
    $.ajax({
      type: "GET",
      url: API_URL,
      ...
      success: async function (data) {
        ...
        var solda = await contract.methods.tokensSold().call();
        document.getElementById("Sold").innerHTML =
          (solda / 1000000000000000000).toFixed(2) + " MOOND";
        (parseInt(data.binancecoin.usd) / parseInt(4224)) * (1 - bon / 10000);

        document.getElementById("MOONDPRICE").innerHTML =
          ((data.binancecoin.usd / 4224) * (1 - bon / 10000)).toFixed(4) + "$";
      },
    });
  };
}
...
NioApp = (function (NioApp, $, window, document) {
  ...
  NioApp.Util.progressBar = function () {
    ...
    if ($data_percent.exists()) {
      $data_percent.each(async function () {
        var sold = await contract.methods.tokensSold().call();
        var precentege = (sold / 1650000000000000000000000) * 100;

        $(this).css("width", precentege + "%");
      });
    }
    // @since v1.6
      ...
      $carousel.each(function () {
        ...
        var c_auto = $self.data("auto") ? $self.data("auto") : false,
          ...
          c_animate_in = $self.data("animate-in")
            ...
            : false;
        var c_start = $self.data("start-element")
          ? $self.data("start-element")
          : 0;
        var c_stage_class =
          "owl-stage " +
          ($self.data("stage-class") ? $self.data("stage-class") : "");
        ...
        $self.addClass("owl-carousel").owlCarousel({
          ...
          startPosition: c_start,
          stageClass: c_stage_class,
          autoHeight: false,
          responsive: {
